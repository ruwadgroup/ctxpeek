...
export async function runDoctor(argv: ReadonlyArray<string> = []): Promise<DoctorReport> {
  ...
  if (isWindows) {
    recommendation =
      "On Windows, configure your MCP client to spawn docpilot via cmd /c — paste:\n" +
      JSON.stringify({ command: "cmd", args: ["/c", "npx", "-y", "docpilot"] }, null, 2);
  }
...
async function checkGithubReachable(token: string | undefined): Promise<DoctorCheck> {
  const rest = new GithubRestClient({ token, userAgent: "docpilot-doctor" });
  try {
